...
### 1. 기본 워크플로우 (단계별 상세 안내)

#### Step 1: 스토리 입력
1. 웹사이트 접속
2. **"스토리 전체를 입력하세요"** 텍스트 영역에 YouTube 영상의 전체 스토리를 입력
   - 예시 텍스트가 이미 입력되어 있습니다
   - 자신의 스토리로 변경하거나 예시 그대로 테스트 가능

#### Step 2: 제목 추천 (선택 사항)
1. **"AI 제목 추천"** 버튼 클릭 (제목 입력란 오른쪽 상단)
2. AI가 추천한 5개의 제목 중 하나 선택
3. 선택한 제목이 자동으로 입력란에 적용됨

#### Step 3: AI 씬 분석
1. **"AI 씬 분석 & 이미지 생성"** 버튼 클릭 (보라색 큰 버튼)
2. AI가 스토리를 자동으로 분석하여 씬(장면)으로 분할
3. 분석 결과가 화면에 표시됨:
   - 각 씬의 설명
   - 지속 시간
   - 시각적 요소
...
#### Step 4: 이미지 생성 시작
1. 씬 분석 결과 하단의 **"이미지 생성 시작"** 버튼 클릭 (초록색 버튼)
   - ⚠️ **중요**: 이 버튼은 씬 분석 완료 후에만 표시됩니다
   - 만약 버튼이 보이지 않는다면 Step 3부터 다시 진행하세요
2. 확인 팝업에서 "확인" 클릭
3. 각 씬마다 이미지가 순차적으로 생성됩니다 (씬당 2-3분 소요)
4. 생성 진행 상황이 실시간으로 표시됩니다

#### Step 5: 개별 이미지 조정
1. 생성 완료 후 각 이미지 카드 확인
2. 마음에 들지 않는 이미지는 **"재생성"** 버튼 클릭
3. 재생성된 이미지로 교체됨

#### Step 6: 전체 다운로드
1. 상단의 **"전체 다운로드 (ZIP)"** 버튼 클릭
2. 모든 이미지가 ZIP 파일로 압축되어 다운로드됨
3. 파일명: scene_01.png, scene_02.png, ...

#### Step 7: 썸네일 생성
1. 모든 씬 이미지 생성 완료 후
2. 하단의 **"썸네일 생성하기"** 버튼 클릭
